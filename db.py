...
def get_subject_id_by_name(subject_name):
    try:
        ...
        with sqlite3.connect("info.db") as connection:
            ...
            cursor.execute(
                "SELECT subject_id FROM all_subjects WHERE subject_name = ?",
                (subject_name,),
            )
...
def get_classes_by_subject_phase(subject_name, phase):
    ...
    try:
        if phase == "Sáng":
            cursor.execute(
                "SELECT * FROM classes WHERE class_name = ? AND time_start < '12:30' ORDER BY teacher",
                (subject_name,),
            )
        elif phase == "Chiều":
            cursor.execute(
...
def get_classes_by_subject_weekday(subject_name, weekday):
    ...
    try:
        cursor.execute(
            "SELECT * FROM classes WHERE class_name = ? AND weekday = ? ORDER BY teacher",
            (subject_name, weekday),
        )
        data = cursor.fetchall()
    except:
...
def get_classes_by_subject_teacher(subject_name, teacher):
    ...
    try:
        cursor.execute(
            "SELECT * FROM classes WHERE class_name = ? AND teacher = ? ORDER BY teacher",
...
def get_subject_id_by_faculty_subject_name(faculty_name, subject_name):
    try:
        with sqlite3.connect("info.db") as connection:
            cursor = connection.cursor()
            cursor.execute(
                "SELECT subject_id FROM subjects_by_faculty_semester WHERE faculty_name = ? AND subject_name = ?",
...
def get_subjects_by_faculty_semester(faculty_name, semester):
    try:
        with sqlite3.connect("info.db") as connection:
            cursor = connection.cursor()
            cursor.execute(
                "SELECT subject_name FROM subjects_by_faculty_semester WHERE faculty_name = ? AND semester = ?",
...
def get_semester_by_faculty(faculty_name):
    try:
        with sqlite3.connect("info.db") as connection:
            cursor = connection.cursor()
            cursor.execute(
                "SELECT DISTINCT semester FROM subjects_by_faculty_semester WHERE faculty_name = ?",
                (faculty_name,),
            )
            data = [item[0] for item in cursor.fetchall()]
    except sqlite3.Error as e:
        ...
        with sqlite3.connect("info.db") as connection:
            ...
            for classes_by_semester in class_info:
                for class_by_semester in classes_by_semester:
                    cursor.execute(
                        "INSERT INTO subjects_by_faculty_semester (faculty_name, semester, subject_id, subject_name) VALUES (?,?,?,?)",
                        (
                            faculty_name,
                            class_by_semester[0],
                            class_by_semester[1],
    ...
    try:
        with sqlite3.connect("info.db") as connection:
            ...
            for faculty_id, faculty_name in faculty_info:
                cursor.execute(
                    "SELECT * FROM faculties WHERE faculty_id = ?", (faculty_id,)
                )
...
def find_subject(text):
    with sqlite3.connect("info.db") as connection:
        ...
        try:
            cursor.execute(
                "SELECT * FROM all_subjects WHERE subject_name LIKE ?",
                ("%" + text + "%",),
            )
            data = cursor.fetchall()
            print(data)
...
def update_all_subjects_table(data=[]):
    try:
        with sqlite3.connect("info.db") as connection:
            ...
            for subject in data:
                cursor.execute(
                    "SELECT * FROM all_subjects WHERE subject_id = ?", (subject[0],)
                )
                if not cursor.fetchone():
                    cursor.execute(
                        "SELECT * FROM all_subjects WHERE subject_name = ?",
                        (subject[1],),
                    )
                    if not cursor.fetchone():
                        cursor.execute(
                    ...
                    cursor.execute(
                        "UPDATE all_subjects SET subject_name = ? WHERE subject_id = ?",
                        (subject[1], subject[0]),
                    )
            connection.commit()
    except sqlite3.Error as e:
    ...
    with sqlite3.connect("info.db") as connection:
        ...
        try:
            ...
            if course:
                # Mã môn đã có
                cursor.execute(
                    "DELETE FROM classes WHERE class_name = ?", (subject_name,)
                )
                for class_info in class_list:
                    cursor.execute(
                        "INSERT INTO classes (subject_id,class_id,class_name,teacher,weekday,time_start,time_end,room) VALUES (?,?,?,?,?,?,?,?)",
                        class_info,
                    )
            else:
                # Mã môn chưa có
                cursor.execute(
                    "INSERT INTO subjects VALUES (?,?)", (subject_id, subject_name)
                )
                for class_info in class_list:
                    cursor.execute(
                        "INSERT INTO classes (subject_id,class_id,class_name,teacher,weekday,time_start,time_end,room) VALUES (?,?,?,?,?,?,?,?)",
...
def get_classes(subject_name=None):
    connection = sqlite3.connect("info.db")
    cursor = connection.cursor()
    try:
        if subject_name:
            cursor.execute(
                "SELECT * FROM classes WHERE class_name = ? ORDER BY teacher",
                (subject_name,),
            )
            data = cursor.fetchall()
        else:
...
def class_count(class_name):
    ...
    try:
        cursor.execute(
            "SELECT COUNT(*) FROM classes WHERE class_name = ?", (class_name,)
        )
...
def get_teachers(subject_name):
    ...
    try:
        cursor.execute(
            "SELECT DISTINCT teacher FROM classes WHERE class_name = ? AND teacher IS NOT NULL",
            (subject_name,),
        )
        data = cursor.fetchall()
    except:
...
def get_weekdays(subject_name):
    ...
    try:
        cursor.execute(
            "SELECT DISTINCT weekday FROM classes WHERE class_name = ? AND weekday IS NOT NULL ORDER BY weekday",
            (subject_name,),
        )
        data = cursor.fetchall()
    except:
